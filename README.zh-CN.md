...
保存是**替换**文件而不是就地改写，这正是让失败或中断的写入不破坏原有内容的原因：替换会保留目标的权限位，以及在 hub 有权设置时保留其 owner；而文件其余随原 inode 存在的东西——ACL 与扩展属性——不会随之保留。一个文件若有多个名字（硬链接）指向它，写入会被拒绝，直到你确认：替换写入的是你保存的那个名字，其他名字仍保留它们原有的内容。
