...
- 副标题不取代现有的「N windows · attached/detached」元信息行；两者是列表项内的不同行。
...
- `hub/internal/tmux/` 新增 pane 列举与解析（格式串、分隔符处理，沿用既有 `parse.go` 的从右切分约定，因为会话名可能含分隔符）。
- `hub/internal/session/` 的 `Backend` 接口与 `Service` 增加 pane 摘要能力，沿用既有 `fastGetter` 式的可选能力断言，避免强制所有 backend 实现。
- `hub/internal/transport/http/dto.go` 的会话列表 DTO 增加可选字段；既有字段与响应结构保持兼容。
...
- 不改变 WebSocket 协议、终端 attach 行为、会话增删改语义或既有 REST 响应字段。
