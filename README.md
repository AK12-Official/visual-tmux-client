...
Saving a file is optimistic: the hub refuses a write whose observed modification time no longer matches, and it re-checks immediately before replacing the file that the file is still the one the write began against. That last check applies to a forced overwrite too, so a file renamed or removed while an upload is in flight is not recreated at its old path. Modification times are compared at the filesystem's own precision where it has one, so two changes inside a single millisecond are two changes.
