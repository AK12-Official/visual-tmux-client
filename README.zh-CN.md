...
浏览器不会只凭文件名就断定一个文件是什么。扩展名暗示二进制、或图片大到无法渲染时，会先向 hub 问一次（一次不携带文件内容的请求），因此名为 `notes.dat` 的文本文件会在编辑器中打开，而不是因为名字被拒。图片预览上限按读取时 hub 报告的大小判定，而不是目录列举时给出的旧大小；超限的文件不会被传输超过上限的部分，而是作为文件信息呈现并提供下载。hub 的回答只回答「这些字节能否按文本解码」这一个问题，因此它决定的是哪些文件进编辑器：上限之内的图片仍然按图片预览，与 hub 是否把它的内容判为二进制无关（图片本来都是二进制）。
