...
- [ ] 9.5 A second round: closure verification that each round-1 fix is complete, and adversarial re-review of the fixes themselves (round 1 produced one regression — the shrink-direction framing error — so the fixes are the place to look).
