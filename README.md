...
`roots` governs what a **caller may name**, and that is the whole of what it guarantees. It is not a containment guarantee against another process on the same machine. Each operation authorizes the target's resolved path and then acts on that path, and the kernel resolves the path again when the call is made — so a local process that can write to a directory the hub traverses can replace a component of the path with a symbolic link in between and have the operation follow it, reaching what the hub's user can read. That matters where the hub runs as a more privileged account than the people who can write to those directories. Closing it needs descriptor-relative access throughout (`openat`-relative walks, or an equivalent rooted handle), which this hub does not do yet; it is recorded as a limit in `openspec/specs/file-manager/spec.md` rather than left implicit.
