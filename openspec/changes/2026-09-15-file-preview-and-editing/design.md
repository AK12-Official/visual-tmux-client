...
What the replacement does *not* carry, because it is a new file rather than an in-place edit: the original's owner, its extended attributes and ACLs, its other hard links, and its setuid/setgid bits. Permission bits are copied onto the new file, and a new file's mode is applied at creation so the process's umask governs it exactly as it governs a file created directly. Carrying setuid across would not preserve a capability but transfer one, since the replacement is owned by whoever wrote it — dropping it is what a shell redirect does. The rest is inherent to the atomic-replace design and is the price of never leaving a partial file at the target.
...
New modules under `hub/web/src/files/`: `api.ts` (reusing `authFetch`/`errorText` from `api.ts`), `pathUtils.ts`, `binaryExtensions.ts`, `preview.ts` (pure dispatch logic + the sanitizing render), and the components `FileManagerOverlay.vue`, `FileTree.vue`, `EditorTabs.vue`, `CodeEditor.vue`, `ImagePreview.vue`, `MarkdownPreview.vue`, `FileContextMenu.vue`. There is no `FileTreeNode.vue`: the tree renders a flat list of visible rows rather than nesting components (see 6.3 in `tasks.md`).
...
The browser requests `GET /api/hosts/{hostId}/sessions/{name}/working-directory` when the manager is opened, and uses the result as the initial directory. The hub answers it with `tmux display-message -p -t =<name>: '#{pane_current_path}'`, built by the existing `ExactTarget` helper so the session name is never treated as a prefix, plus the trailing colon that makes the target a pane rather than a session (see 4.5 in `tasks.md`). This is the navigation seed of decision 5, not an authorization input.
...
The global `MaxRequestBodyBytes` (64 KiB, `router.go:17`) is far too small for a file write. The `PUT .../write` handler installs its own `http.MaxBytesReader` derived from `files.max_file_size`, rather than raising the global limit — raising it would weaken every other endpoint. There is no allowance added on top: the declared `size` is checked separately against the same limit by the service, so a body at the limit is accepted and one byte over is refused, and the two bounds agree by construction.
...
- **[Risk] A path can be redirected between the check and the syscall, because the kernel re-walks every component of the string it is handed.** → *Partly mitigated, and the remainder accepted with its cost stated.* Resolution happens once and the canonical result is what gets authorized, so a link that is merely *swapped* cannot redirect the operation; `Delete` and `Rename` additionally express the operation relative to the resolved parent, so no link remains in the string they act on and the final element is never followed. What remains is a real-directory component being *replaced* by a link: escaping that needs concurrent local write access to a directory on the path plus the ability to create symlinks there — and the consequence is reaching data **outside** the boundary, which is the access being denied, not a weaker version of it. So the mitigation is not that the attacker already has what they reach; it is that this needs a concurrent local actor rather than the token alone, and that closing it fully requires `openat`-relative access throughout (`os.Root`, or component-wise `O_NOFOLLOW` walks) — noted, not built. The regression test for the link case is probabilistic by nature: read a green run as evidence, not proof.
- **[Known limit] Containment is decided before existence only for a path that does not exist.** A path whose *shape* the kernel rejects first — a component that is not a directory, or a link leading in a circle — is refused for that reason rather than as outside the boundary, so a caller can tell those apart for a path outside every root. That is one bit about a path the caller may not name, and nothing more: no path outside a root becomes reachable, readable, or writable, and the distinction predates this change (before the errno mapping it was `500` versus `403`). Closing it would mean answering every unclassifiable path with "not allowed", which would hide the reason from callers naming perfectly good paths *inside* the boundary — a worse trade.
...
- **[Resolved] Whether `files.enabled: false` should also hide the toolbar button entirely or leave it disabled with an explanation.** Hidden. The hub publishes `files.enabled` (and `files.max_file_size`) in the unauthenticated client configuration, and the button is left out when it is false, since a disabled button with no explanation invites support questions. The flag is a capability report, not a boundary: every operation is still gated by the hub on its own. The published limit is what lets a refusal say "larger than this hub's 100 MiB limit" instead of naming a wire code.
