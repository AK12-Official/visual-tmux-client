...
`roots` 约束的是**调用方可以指定什么路径**，这也是它全部的作用。它不构成对同机其他进程的隔离保证：每次操作都先对解析后的真实路径做鉴权，然后按该路径执行，而内核在真正发起系统调用时会再解析一次——因此一个能写入 hub 会经过的某个目录的本地进程，可以在两次解析之间把路径中的某一段替换为软链，让操作跟过去，从而读到 hub 用户可读的内容。这在 hub 以权限更高、而那些人可写的目录又较多的账号运行时才有实际意义。要彻底关闭它，需要全程使用基于文件描述符的相对访问（`openat` 相对遍历，或等价的 root 句柄），本 hub 尚未实现；该限制已明确记录在 `openspec/specs/file-manager/spec.md`，而不是留作隐含假设。
