...
配置了 `roots` 时，操作不仅会**按边界鉴权**，还会**通过一个打开在所属 root 上的句柄、以相对该 root 的路径执行**。这正是让边界对同机其他进程也成立、而不只是对调用方成立的关键：鉴权会解析目标路径，而在这之后、操作之前被替换为、且指向 root 之外的软链，会被句柄拒绝，而不是被悄悄跟随；若替换后的链接仍指回同一个 root 内，则会被跟随——句柄拒绝的是越出边界，而不是被重定向。唯一例外是在两个已配置 root 之间移动条目——句柄只能在自己的树内移动——它按两个绝对路径执行，期间被替换的路径段仍可能改变移动的落点。
...
浏览器不会只凭文件名就断定一个文件是什么。扩展名暗示二进制、或图片大到无法渲染时，会先向 hub 问一次（一次不携带文件内容的请求），因此名为 `notes.dat` 的文本文件会在编辑器中打开，而不是因为名字被拒。图片预览上限按读取时 hub 报告的大小判定，而不是目录列举时给出的旧大小；超限的文件不会被传输超过上限的部分，而是作为文件信息呈现并提供下载。
...
保存采用乐观并发：观察到的修改时间不再匹配时 hub 会拒绝写入，并在真正替换文件之前再确认一次目标仍是本次写入开始时的那一个。这最后一道检查同样作用于强制覆盖，因此在传输途中被改名或删除的文件不会被原路径重新创建出来。修改时间的比较使用文件系统自身的精度，因此同一毫秒内的两次改动会被视为两次改动。
