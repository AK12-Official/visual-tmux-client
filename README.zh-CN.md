...
保存采用乐观并发：观察到的修改时间不再匹配时 hub 会拒绝写入，并在真正替换文件之前再确认一次目标仍是本次写入开始时的那一个。这最后一道检查同样作用于强制覆盖，因此在传输途中被改名或删除的文件不会被原路径重新创建出来。修改时间的比较使用文件系统自身的精度，因此同一毫秒内的两次改动会被视为两次改动。管理器还会让自己的两个请求互相避让：删除会等待已在途的保存结束后才发出，而对一个已发出删除的文件进行保存会被拒绝而不是发出。这个次序覆盖的是本浏览器自己发出的写入；其他程序发起的写入不受它约束，上面那道最终检查正是为此存在。
