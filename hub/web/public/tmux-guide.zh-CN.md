...
| 改名 | 卡片上的 ✎；支持中文等非 ASCII 字符（最多 64 字符），不能包含 `:`、`.`、`/`、`\`、`;`、全角 `：`/`．`/`；` 及首尾空白 |
| 杀死会话 | 卡片上的 ✕（会确认） |
...
点击左侧会话即附加。顶部栏从左到右：活动指示点、会话名、连接状态，右侧 `A−` / 字号 / `A+` 调整字号、`Files` 打开文件管理器、`⛶` 全屏、`✕` 关闭面板。

### 文件管理器

点顶栏的 `Files` 在终端区域内打开，不会遮住左侧会话列表。它会打开在当前会话活动 pane 的工作目录，之后可以自由上下浏览——之后再切换 pane 不会让已经打开的管理器跟着跳。

左侧是目录树，展开子目录时才按需加载。右侧可同时打开多个文件，每个标签独立标记未保存状态（标题旁的 `●`）。保存时会带上你打开该文件时看到的修改时间；若文件在期间被外部改动，hub 会拒绝并询问是否覆盖。

Markdown 文件默认以源码打开，可切到 `Preview` 查看渲染结果（内容经消毒后才插入页面，脚本与外链加载都被去掉）。图片直接预览；二进制或过大的文件显示文件信息与下载按钮，不会当作文本打开。

在目录树的条目上点右键可以新建文件/目录、重命名、删除（会先确认）、下载、复制路径，以及**把路径插入终端输入行**——插入的只是一个路径文本，不带换行，因此不会自己执行；含空格的路径会自动加引号。若当前没有已附加的终端，会提示而不是静默失败。
...
Visual Tmux Client 会在连接和创建会话时**默认自动开启鼠标支持**（`set -g mouse on`），无需手动设置。若在原生终端中手动开启，可在 `:` 提示符中输入 `:set -g mouse on`。

开启后支持以下操作：

- **鼠标滚轮 / 触摸板滑动** → 当会话输出超过一屏时，向上滑动直接翻阅历史上下文（tmux 自动进入复制模式），向下滑动到底部自动退出复制模式并恢复交互命令行
- **左键点击窗格** → 切换活动窗格
- **左键点击状态栏窗口名** → 切换窗口
- **在窗格边框上左键拖动** → 调整窗格大小
- **在窗格内左键拖动** → 选中文本（松开即复制到 tmux 缓冲）
- **按住 Shift 键拖动鼠标** → 选中文本至浏览器（配合 `Ctrl+C` / `Cmd+C` 复制到系统剪贴板）
- **右键点击窗格** → 弹出右键快捷菜单
