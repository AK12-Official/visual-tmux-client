...
配置了 `roots` 时，操作不仅会**按边界鉴权**，还会**通过一个打开在所属 root 上的句柄、以相对该 root 的路径执行**。这正是让边界对同机其他进程也成立、而不只是对调用方成立的关键：鉴权会解析目标路径，而在这之后、操作之前被替换为软链的某一段路径，会被句柄拒绝，而不是被悄悄跟随到 root 之外。唯一例外是在两个已配置 root 之间移动条目——句柄只能在自己的树内移动——它按两个绝对路径执行，期间被替换的路径段仍可能改变移动的落点。
